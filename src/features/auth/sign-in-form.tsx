    ...
    if (userId) {
      ...
      if (prof?.handle) {
        const { data: st } = await supabase
          .from("startups")
          .select("id,slug,name,tagline,about,logo_url,website,city,topics,founded_year,team_size,service_category,studio_status,details")
          .eq("owner_id", userId)
          .maybeSingle();
        ...
        hydrateFromRemote({
          ...
          startup: st
            ? {
                ...
                studioStatus: (st.studio_status ?? undefined) as
                  | "pending"
                  | "approved"
                  | "rejected"
                  | undefined,
